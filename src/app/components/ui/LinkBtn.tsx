...
const LinkBtn = () => {
  ...
  return (
    <div className={`flex flex-col items-center gap-5 fixed top-100 z-1000 ${language === 'en' ? 'right-0 sm:right-7' : 'left-0 sm:left-5'}`}>
      ...
      <p className={`${language === 'en' ? 'py-5 rotate-90' : 'py-7 -rotate-90'} text-lg`}>{language === 'en' ? "Let's Talk" : "دعنا نتواصل"}</p>
    </div>
  );
};
