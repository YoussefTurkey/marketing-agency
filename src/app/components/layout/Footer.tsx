...
const Footer = () => {
  ...
  return (
    <footer className={`container mx-auto flex flex-col items-center w-[95%] xl:w-full px-10 py-5 backdrop-blur-sm rounded-lg border-[1px] ${theme === 'dark' ? 'border-[#ffffff30] bg-white/10' : 'border-[#00000030] bg-black/5'}`}>
      <div className="flex flex-col md:flex-row gap-5 items-center justify-between text-center md:text-left w-full">
        ...
        <div className="flex flex-col gap-5 items-center">
          ...
          <div className="flex items-center gap-5">
            <Link href={'/'} target="_blank"><FaFacebookSquare size={22.5} /></Link>
            <Link href={'/'} target="_blank"><BsTwitterX size={20} /></Link>
            <Link href={'/'} target="_blank"><FaInstagramSquare size={22.5} /></Link>
            <Link href={'/'} target="_blank"><ImLinkedin size={20} /></Link>
          </div>
        </div>
      </div>
